...
export default function
  Button({ label, mode = 'lg', btnVariant = `red`, linearGradient, disabled = false, ...props }: ButtonType) {
      ...
      {(mode === 'md' && btnVariant === `dark-blue`) && (
        <button disabled={disabled} {...props} className={`font-bold bg-white text-white rounded-2xl flex 
              ...
              transition-all duration-200 
               ${disabled ? `cursor-not-allowed bg-zinc-400` : `bg-linear-main-dark-blue hover:bg-zinc-900`}`}> {label} </button>
      )}
    </>
  );
}
...
export const leavePropertyReviewSchema = z.object({
  ...
  noiseLevel: z.number({ message: 'The NoiseLevel should be be a valid number!' }).min(1, 'NoiseLevel cannot be less than 1.').max(5,
    'Location cannot be higher than 5.'),
  ownership: z.number({ message: 'The Location should be be a valid number!' }).min(1, 'Location cannot be less than 1.').max(5,
    'Location cannot be higher than 5.'),
  amenities: z.number({ message: 'The Location should be be a valid number!' }).min(1, 'Location cannot be less than 1.').max(5,
    'Location cannot be higher than 5.'),
